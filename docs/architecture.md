...
Once connected, a Root/Item has `data-interactive`. The demo CSS uses `data-open` as its styling hook. A separate fallback reads `hidden` only before initialization so native fragment reveals work without JavaScript. Triggers gain button semantics and Space activation. Successful ordinary activation prevents fragment navigation; modified clicks retain native link behavior. Demo transitions become enabled with `data-interactive`; there is no separate motion state. The docs share their actual baseline/animation stylesheet with their code examples.
