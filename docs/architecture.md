...
Once connected, a Root/Item has `data-interactive`. The demo CSS uses `hidden` for visibility both before and after initialization; the primitive also exposes `data-open` for styling. Triggers gain button semantics and Space activation. Successful ordinary activation prevents fragment navigation; modified clicks retain native link behavior. Initial setup does not enable motion. The first enhanced toggle sets `data-motion`, allowing consumer animation styles. Accordion shares this motion state across its items so outgoing and incoming panels animate together. The docs share their actual baseline/animation stylesheet with their code examples.
